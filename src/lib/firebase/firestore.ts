...
function logFirestoreError(label: string, meta: FirestoreDebugMeta, error: unknown) {
  const authUid = firebaseAuth.currentUser?.uid ?? null;
  const authAnonymous = firebaseAuth.currentUser?.isAnonymous ?? null;
  console.error(`[firestore] ${label} failed`, {
    ...meta,
    authUid,
    authAnonymous,
...
async function withFirestoreDebug<T>(label: string, meta: FirestoreDebugMeta, op: () => Promise<T>): Promise<T> {
  try {
    return await op();
  } catch (error) {
    logFirestoreError(label, meta, error);
    throw error;
  }
}

export async function fetchUserProfile(uid: string): Promise<UserProfile | null> {
  return withFirestoreDebug('fetchUserProfile', { uid, collection: 'users' }, async () => {
...
export async function listMainCareers(uid: string): Promise<MainCareer[]> {
  return withFirestoreDebug('listMainCareers', { uid, collection: 'mainCareers' }, async () => {
...
export async function createAndActivateMainCareer(params: { uid: string; title: string }): Promise<string> {
  ...
  return withFirestoreDebug('createAndActivateMainCareer', { uid, title, collection: 'mainCareers' }, async () => {
...
export async function activateMainCareer(params: { uid: string; mainCareerId: string }) {
  ...
  return withFirestoreDebug(
    'activateMainCareer',
    { uid, mainCareerId, collection: 'mainCareers' },
    async () => {
      const userRef = doc(firestore, 'users', uid);
...
export async function archiveActiveMainCareer(uid: string) {
  return withFirestoreDebug('archiveActiveMainCareer', { uid, collection: 'mainCareers' }, async () => {
    const userRef = doc(firestore, 'users', uid);
    const activeSessionRef = doc(firestore, 'users', uid, 'active_session', 'current');
    const nowTimestamp = Timestamp.now();
    await runTransaction(firestore, async (tx) => {
...
export async function updateMainCareerTitle(params: { uid: string; mainCareerId: string; title: string }) {
  ...
  return withFirestoreDebug(
    ...
    { uid, mainCareerId, title, collection: 'mainCareers' },
    async () => {
      const trimmedTitle = title.trim();
      if (trimmedTitle.length < 2 || trimmedTitle.length > 30) {
        throw new Error('主线名称需为 2~30 个字符');
...
export async function startActiveSession(params: {
  ...
  return withFirestoreDebug(
    'startActiveSession',
    { uid, mainCareerId, deviceId, collection: 'users/active_session' },
    async () => {
      const activeRef = doc(firestore, 'users', uid, 'active_session', 'current');
      const userRef = doc(firestore, 'users', uid);
      const careerRef = doc(firestore, 'mainCareers', mainCareerId);
...
export async function heartbeat(uid: string, deviceId: string) {
  return withFirestoreDebug('heartbeat', { uid, deviceId, collection: 'users/active_session' }, async () => {
    const activeRef = doc(firestore, 'users', uid, 'active_session', 'current');
...
export async function endActiveSession(params: {
  ...
  return withFirestoreDebug(
    'endActiveSession',
    { uid, deviceId, timeZone, collection: 'sessions' },
    async () => {
      const activeRef = doc(firestore, 'users', uid, 'active_session', 'current');
      const userRef = doc(firestore, 'users', uid);
      const now = new Date();
...
export async function querySessionsByDay(uid: string, dayKey: string): Promise<FocusSession[]> {
  return withFirestoreDebug('querySessionsByDay', { uid, dayKey, collection: 'sessions' }, async () => {
...
export async function queryAllSessions(uid: string): Promise<FocusSession[]> {
  return withFirestoreDebug('queryAllSessions', { uid, collection: 'sessions' }, async () => {
    const sessionsRef = collection(firestore, 'sessions');
...
export async function queryDailyTotals(params: {
  ...
  return withFirestoreDebug(
    'queryDailyTotals',
    { uid, lastNDays, timeZone, collection: 'daily_stats' },
    async () => {
...
export async function queryWeeklyStats(params: { uid: string; lastNWeeks?: number }): Promise<WeeklyStat[]> {
  ...
  return withFirestoreDebug(
    'queryWeeklyStats',
    { uid, lastNWeeks, collection: 'weekly_stats' },
    async () => {
      const startDate = new Date();
...
export async function fetchGlobalStats(uid: string): Promise<GlobalStat | null> {
  return withFirestoreDebug('fetchGlobalStats', { uid, collection: 'global_stats' }, async () => {
    const globalRef = doc(firestore, 'global_stats', uid);
